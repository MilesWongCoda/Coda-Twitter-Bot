...
class EveningWrapJob(BaseJob):
    def __init__(self, store, generator, poster, summarizer: Summarizer,
                 ...
                 polymarket=None, macro=None, synthesizer_data=None, notifier=None):
    ...
    def execute(self):
        if self.store.topic_on_cooldown("evening_wrap", cooldown_hours=20):
            return None
        ...
        parts = [f"End of day wrap. Prices: {price_summary}"]
        if fg:
            parts.append(fg)
        parts.append(f"Key events: {news_summary}")

        if self.etf_flows:
            s = self._safe_format(self.etf_flows, self.etf_flows.get_etf_snapshot())
            if s:
                parts.append(f"Daily ETF scorecard: {s}")
        ...
        if self.whale_alerts:
            ...
            if s:
                parts.append(f"Whale activity: {s}")
        if self.polymarket:
        ...
        top = self.get_top_performers(job_type="evening_wrap")
        bottom = self.get_bottom_performers(job_type="evening_wrap")
        ...
        try:
            ...
            chart_path = generate_market_card(price_data, fg_raw, "evening_wrap")
            if chart_path:
                try:
        ...
        # Friday: mini-thread mode (week close)
        if self._should_use_thread(days=(4,)):
            try:
                tweets = self.generator.generate_mini_thread(
                    context, "evening_wrap", num_tweets=3,
                ...
                if tweets and len(tweets) >= 2:
                    for t in tweets:
                        self._validate_output(context, t, "evening_wrap")
                    tweet_ids = self.poster.post_thread(tweets, first_tweet_media_ids=media_ids)
                    if tweet_ids:
                        for tid, txt in zip(tweet_ids, tweets):
                            self._mark_posted_with_topics(tid, "evening_wrap", txt, topic="evening_wrap")
                        self._schedule_followup(tweet_ids[0], context, "evening_wrap", tweet_text=tweets[0])
                        return tweet_ids
            except Exception as exc:
                logger.warning("Mini-thread failed for evening_wrap, falling back to single: %s", exc)

        # Single tweet (default or fallback)
        tweet = self._generate_with_dedup(context, "evening_wrap", recent, top=top, bottom=bottom, trending_tags=trending_tags, performance_patterns=patterns)
        if not tweet:
            return None
        self._validate_output(context, tweet, "evening_wrap")
        ...
        self._mark_posted_with_topics(tweet_id, "evening_wrap", tweet, topic="evening_wrap")
        self._schedule_followup(tweet_id, context, "evening_wrap", tweet_text=tweet)
        return tweet_id
